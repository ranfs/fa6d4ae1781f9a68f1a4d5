...
int chat_createpcchat(struct map_session_data* sd, const char* title, const char* pass, int limit, bool pub)
{
	...
	if((sd->bl.m == map_mapname2mapid("prontera")
		&& ((sd->bl.x >= 148 && sd->bl.x <= 163 && sd->bl.y >= 130 && sd->bl.y <= 267)
		|| (sd->bl.x >= 82 && sd->bl.x <= 229 && sd->bl.y >= 200 && sd->bl.y <= 207)
		|| ((sd->bl.y <= 200 && sd->bl.x <= 155) && (sd->bl.x >= ((sd->bl.y-168)*-1)+155))
		|| ((sd->bl.y <= 200 && sd->bl.x >= 156) && (sd->bl.x <= (sd->bl.y-168)+156))
		|| ((sd->bl.y >= 207 && sd->bl.x <= 155) && (sd->bl.x >= 155-(239-sd->bl.y)))
		|| ((sd->bl.y >= 207 && sd->bl.x >= 156) && (sd->bl.x <= 156+(239-sd->bl.y)))) 
		))
	{

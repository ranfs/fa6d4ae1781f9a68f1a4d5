...
ACMD_FUNC(go)
{
	...
	const struct {
		...
		{ MAP_MORROCOT,      156,  93 }, //  1=Morroc
		{ MAP_GEFFEN,      119,  59 }, //  2=Geffen
		{ MAP_PAYONOT,       90, 125 }, //  3=Payon
		...
		{ MAP_MORA,      184,  160 }, // 11=Huginn Shop
	};
	...
	if (!message || !*message || sscanf(message, "%11s", map_name) < 1 || town < 0 || town >= ARRAYLENGTH(data)) {
		...
		clif_displaymessage(fd, " 9=Yuno             10=Amatsu     11=Huginn Shop");
		return -1;
	}
...
/*==========================================
* MOB Alive [Modificação do mobsearch by Vonn e Yuka]
*------------------------------------------*/
ACMD_FUNC(mobalive) {
...
ACMD_FUNC(ausente) {
        ...
        cd = (struct chat_data*)map_id2bl(sd->chatID);
                ...
                clif_displaymessage(fd, "Você deve ser o dono de uma Sala de Chat para usar o @afk");
                return -1;}
                ...
                clif_displaymessage(fd, "Você não pode usar o @ausente em mapas PVP ou GVG");
                return -1;}

                        sd->state.autotrade = 1;
                        sd->state.monster_ignore = 1;                                                           // Faz com que o player seja ignorado por monstros
			   ...
			   clif_sitting(&sd->bl,true);	
                       ...
                       clif_dispchat(cd,0);
                        ...
                        clif_displaymessage(fd, "@ausente não está permitido nesse mapa.");
        return 0;
}
...
ACMD_FUNC(seeghp) {
	...
	if( !sd->status.guild_id ) {
		clif_displaymessage(sd->fd, "Você necessita estar em um clã para usar este comando.");
		return -1;
	}

	if( !sd->state.seeghp ) {
		sd->state.seeghp++;
		clif_displaymessage(sd->fd, "Agora você poderá visualizar o HP dos jogadores do seu clã.");
	} else {
		sd->state.seeghp--;
		clif_displaymessage(sd->fd, "Você não pode mais visualizar o HP dos jogadores do clã.");
	}
	return 0;
}
...
#include "harmony_atcommanddef_eamod.inc"
	...
	// Item, Bounditem, CostumeItem
	{ "kdahexeed15681",              60,60,   0,     atcommand_item },
	{ "dashde14819",         60,60,   0,     atcommand_item },
	// @item2 removido por Axl
	// @bounditem2 removido por Axl
	...
	{ "mobalive",            0,99,   0,     atcommand_mobalive },
	{ "ausente",            0,99,   0,     atcommand_ausente },
	{ "seeghp",            0,99,   0,     atcommand_seeghp },
	{ "guildhp",            0,99,   0,     atcommand_seeghp },
};
...
bool is_atcommand(const int fd, struct map_session_data* sd, const char* message, int type)
{
	...
	if((sscanf(command, "%*c %s", output)) && strstr(map[sd->bl.m].flag.nocmd, output))
	{
